...
class Document extends \DOMDocument
{
    ...
    protected $_nodeClasses = [
        '\\DOMNode' => '\\PHPDOM\\HTML\\Node',
        '\\DOMElement' => '\\PHPDOM\\HTML\\Element',
        '\\DOMText' => '\\PHPDOM\\HTML\\Text',
    ...
    private function _init($has_template)
    {
        ...
        foreach ($this->_nodeClasses as $native => $extended) {
            $this->registerNodeClass($native, $extended);
        }
    ...
    public function create($definition = [])
    {
        ...
        if (empty($normalized->tag)) {
            ...
            $node->attrset->setAll($normalized->attributes);
        }
    ...
    public function addHeadScript($path, $directory = '/js/', array $attributes = [])
    {
        return $this->select('head')->addScript($path, $directory, $attributes);
    }

    public function addBodyScript($path, $directory = '/js/', array $attributes = [])
    {
